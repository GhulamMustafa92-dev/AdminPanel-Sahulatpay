...
import {
  MessageSquareWarning, CheckCircle2, XCircle, Clock, AlertTriangle, X, Check,
...
import {
  getDisputes, reviewDispute,
  type Dispute, type DisputesResponse,
...
const STATUS_BADGE: Record<string, string> = {
  ...
  under_review: "bg-[rgba(249,115,22,0.1)] text-[#f97316] border-[rgba(249,115,22,0.2)]",
  resolved:     "bg-green-500/10 text-green-400 border-green-500/20",
  dismissed:    "bg-[#252525] text-[#888888] border-[#333333]",
};

const TYPE_BADGE: Record<string, string> = {
  unauthorized:     "bg-red-500/10 text-red-400 border-red-500/20",
  wrong_amount:     "bg-[rgba(249,115,22,0.1)] text-[#f97316] border-[rgba(249,115,22,0.2)]",
  wrong_recipient:  "bg-yellow-500/10 text-yellow-400 border-yellow-500/20",
  other:            "bg-[#252525] text-[#888888] border-[#333333]",
};
...
export default function DisputesPage() {
  ...
  const { data, isLoading, refetch } = useQuery<DisputesResponse>({
    ...
    queryFn: () => getDisputes({ page, per_page: PAGE_LIMIT, status: statusFilter }),
  ...
  const reviewMutation = useMutation({
    mutationFn: () =>
      reviewDispute(selectedDispute!.id, decision, adminNote.trim() || undefined),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["disputes"] });
      closeDialog();
      showToast(
        decision === "accept"
          ? "Dispute accepted — reversal has been triggered."
          : "Dispute dismissed successfully.",
        "success"
      );
    },
    onError: () => showToast("Failed to review dispute.", "error"),
  });
  ...
  const confirmReview = () => {
    if (decision === "accept" && adminNote.trim().length < 30) {
      setFieldErr("Accepting requires at least 30 characters of evidence in the admin note.");
      return;
    }
    setFieldErr("");
  ...
  const hasNextPage      = disputes.length >= PAGE_LIMIT;

  const canReview = (d: Dispute) => d.status === "open" || d.status === "under_review";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="font-syne font-bold text-[#f0f0f0] text-xl leading-tight">Disputes</h2>
          <p className="text-[11px] font-dm-sans text-[#888888] leading-none mt-0.5">
            Manage user-reported transaction disputes and resolutions
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <MiniStat
          icon={MessageSquareWarning} label="Open"
          value={openCount}
        ...
        />
        <MiniStat
          icon={XCircle}             label="Dismissed"
          value={dismissedCount}
          iconCls="text-[#888888]"   bgCls="bg-[#252525]"
        />
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-[#111111] border-b border-[#252525]">
              {["Transaction ID", "User ID", "Type", "Reason", "Status", "Created At", "Actions"].map(
                (col, i) => (
                  <th
                    key={col}
                    className={cn(
          ...
          <tbody className="divide-y divide-[#252525]">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={7} className="py-12 text-center bg-[#1a1a1a]">
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={cn(
                        ...
                        TYPE_BADGE[d.dispute_type] ?? "bg-[#252525] text-[#888888] border-[#333333]"
                      )}
                    >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={cn(
                        "inline-flex px-2 py-0.5 rounded-full text-[10px] font-dm-sans font-semibold border capitalize",
                        STATUS_BADGE[d.status] ?? "bg-[#252525] text-[#888888] border-[#333333]"
                      )}
                    >
                  ...
                  <td className="px-4 py-3">
                    {canReview(d) ? (
                      ...
                      >
                        <MessageSquareWarning className="w-3 h-3" /> Review
                      </button>
                    ) : (
      ...
      {/* ── Review Dialog ─────────────────────────────────────────────────── */}
      <Dialog.Root
        open={dialogOpen && !!selectedDispute}
        onOpenChange={(v) => { if (!v) closeDialog(); }}
      >
        <Dialog.Portal>
          <Dialog.Overlay className={OVERLAY_CLS} />
          <Dialog.Content className={CONTENT_CLS} aria-describedby="dispute-review-desc">
            ...
            <Dialog.Description
              id="dispute-review-desc"
              className="text-[13px] font-dm-sans text-[#888888] mb-4"
            >
              <span className="font-mono text-[12px] text-[#f0f0f0]">
                {selectedDispute && trunc(selectedDispute.transaction_id, 12)}
              </span>
              {" · "}
              <span
                className={cn(
                  ...
                  selectedDispute
                    ? (TYPE_BADGE[selectedDispute.dispute_type] ?? "bg-[#252525] text-[#888888] border-[#333333]")
                    : ""
                )}
              >
            ...
            {selectedDispute?.reason && (
              <div className="mb-4 px-3.5 py-3 rounded-xl bg-[#1a1a1a] border border-[#252525]">
                <p className="text-[10px] font-dm-sans font-semibold uppercase tracking-[0.08em] text-[#4a4a4a] mb-1.5">
                  Dispute Reason
                </p>
                <p className="text-[12px] font-dm-sans text-[#f0f0f0] leading-relaxed">
                  {selectedDispute.reason}
                </p>
              </div>
            )}

            {/* Evidence note (if present) */}
            {selectedDispute?.evidence_note && (
              <div className="mb-4 px-3.5 py-3 rounded-xl bg-[rgba(249,115,22,0.05)] border border-[rgba(249,115,22,0.15)]">
                <p className="text-[10px] font-dm-sans font-semibold uppercase tracking-[0.08em] text-[#f97316] mb-1.5">
                  Evidence Note
                </p>
                <p className="text-[12px] font-dm-sans text-[#f0f0f0] leading-relaxed">
                  {selectedDispute.evidence_note}
            ...
            <div className="flex items-start gap-2.5 px-3.5 py-3 mb-4 rounded-xl bg-yellow-500/5 border border-yellow-500/20">
              <AlertTriangle className="w-4 h-4 text-yellow-400 shrink-0 mt-0.5" />
              <p className="text-[12px] font-dm-sans text-yellow-400 leading-relaxed">
                Note: Accepting requires ≥ 30 characters of evidence in the admin note.
              </p>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-3">
                {(["accept", "dismiss"] as const).map((opt) => (
                  <button
                    key={opt}
                    onClick={() => { setDecision(opt); setFieldErr(""); }}
                    className={cn(
                      ...
                      decision === opt
                        ? opt === "accept"
                          ...
                          : "bg-[#252525] border-[#333333] text-[#888888]"
                  ...
                  >
                    ...
                    <span className="capitalize">
                      {opt === "accept" ? "Accept (triggers reversal)" : "Dismiss"}
                    </span>
                  </button>
                ))}
              </div>
            ...
            <div className="mb-5">
              <label className="block text-[10px] font-dm-sans font-semibold uppercase tracking-[0.08em] text-[#4a4a4a] mb-2">
                Admin Note{" "}
                <span className="normal-case font-normal text-[#4a4a4a]">
                  {decision === "accept" ? "(required, ≥ 30 chars)" : "(optional)"}
                </span>
              </label>
              <textarea
                value={adminNote}
                onChange={(e) => { setAdminNote(e.target.value); setFieldErr(""); }}
                rows={4}
                placeholder={
                  decision === "accept"
                    ? "Provide detailed evidence supporting this acceptance…"
                    : "Add a note for dismissing this dispute (optional)…"
                }
                className={cn(
                  ...
                  fieldErr
                    ? "border-red-500/50"
                    : "border-[#252525] focus:border-[rgba(249,115,22,0.4)]"
                )}
              />
              {fieldErr && (
                <p className="text-[11px] font-dm-sans text-red-400 mt-1">{fieldErr}</p>
              )}
              {decision === "accept" && !fieldErr && adminNote.trim().length > 0 && (
                <p className={cn(
                  "text-[11px] font-dm-sans mt-1 transition-colors",
                  adminNote.trim().length >= 30 ? "text-green-400" : "text-[#888888]"
                )}>
                  {adminNote.trim().length} / 30 characters
                </p>
              )}
            </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  decision === "accept"
                    ...
                    : "bg-[#252525] text-[#888888] border-[#333333] hover:bg-[#2a2a2a] hover:text-[#f0f0f0]"
                )}
              >
                {reviewMutation.isPending
                  ...
                  : (
                    <>
                      ...
                      {decision === "accept" ? "Accept & Trigger Reversal" : "Dismiss Dispute"}
                    </>
                  )
                }
              </button>
